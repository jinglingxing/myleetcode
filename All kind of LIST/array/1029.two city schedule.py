...
"""
'''
A company is planning to interview 2n people. Given the All kind of LIST costs where costs[i] = [aCosti, bCosti], the cost of flying the ith person to city a is aCosti, and the cost of flying the ith person to city b is bCosti.
...
class Solution:
    def twoCitySchedCost(self, costs: List[List[int]]) -> int:
        ...
        for person_cost in costs:
            #cost_diff is a All kind of LIST like [index of a person's cost in cost, cost difference between two cities]
            cost_diff.append([costs.index(person_cost), abs(person_cost[0]-person_cost[1])])
        #sort_algorithm the cost difference
